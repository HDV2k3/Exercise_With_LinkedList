...
node *createNode()
{
  ...
  scanf("%s",&sv->name);
    ...
    printf("\nNhap d3: ");
	scanf("%u",&sv->d3)     
   float dtb=(sv->d1*1.0+sv->d2*1.0+sv->d3*1.0)/3;
    sv->next=NULL;
    return sv; 
}

void Insert_first (node* list, node* P)  {
...
{                    
    ...
    for(i=0;i<n;i++){
        node *b = createNode();
        insertFirst(head,id,name,d1,d2,d3);
    }
}
...
void displayLinkedList(struct node* root) {
  ...
  printf("\nDSSV hien tai: ");
  printf("\nSTT \t MSSV \t Name                 \t D1 \t D2 \t D3 \t DTB ");
  int i=1;
  while (temp != NULL) {
  	float dtb=(temp->d1*1.0+temp->d2*1.0+temp->d3*1.0)/3;
    printf("\n ->%d \t %d \t %s \t %d \t %d \t %d \t %.2f",i++, temp -> id, temp -> name, temp -> d1, temp -> d2, temp -> d3,dtb);
    temp = temp -> next;
  }
  printf("\n -> NULL\n\n");
}
...
int main() {
  ...
  head = insertFirst(head, 90101, "Vo Binh An\t",22,52,66);
  head = insertFirst(head, 90102, "Truong An Binh",44,84,64);
  head = insertFirst(head, 90103, "Nguyen Minh Chien",34,21,57);
  head = insertFirst(head, 90104, "Ngo Thi Dung\t",87,34,62);
